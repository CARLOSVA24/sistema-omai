console.log("GESTIÓN DE PATRULLAJES TERRESTRES v2.0 Loaded");
...
function initAuth() {
    let storedPasses = safeJSONParse('app_passwords', null);

    if (!storedPasses || Object.keys(storedPasses).length === 0) {
        storedPasses = defaultPasswords;
        localStorage.setItem('app_passwords', JSON.stringify(defaultPasswords));
    }
    ...
    if (loginSelect) {
        ...
        console.log("Login dropdown populated with roles:", Object.keys(storedPasses));
    } else {
        console.warn("Element 'loginRole' not found during initAuth.");
    }
...
function handleLogin() {
    ...
    const storedPasses = safeJSONParse('app_passwords', defaultPasswords);

    if (storedPasses[role] === pass) {
...
window.handleGenerateKey = function (e) {
    ...
    localStorage.setItem('app_passwords', JSON.stringify(storedPasses));
...
var crimes = safeJSONParse('gyecrimes', []);
var personnel = safeJSONParse('gyepersonal', []);
...
var incidentMarkers = {};
var crimeChart = null;
var guardAssignments = safeJSONParse('guardAssignments', []);
var specialAssignments = safeJSONParse('specialAssignments', []);
var selectedWatchGroup = localStorage.getItem('selectedWatchGroup') || null;
var baborPersonnel = safeJSONParse('baborPersonnel', []);
var estriborPersonnel = safeJSONParse('estriborPersonnel', []);
var opsEvents = safeJSONParse('opsEvents', []);
var instantOps = safeJSONParse('instantOps', []);
var patrolOrders = safeJSONParse('patrolOrders', []);
var commandPostPersonnel = safeJSONParse('commandPostPersonnel', []);
var personnelHistory = safeJSONParse('personnelHistory', []);
var vehicles = safeJSONParse('gyevehicles', []);
var operationName = localStorage.getItem('operationName') || "S/N";
var externalOrdersMetadata = safeJSONParse('externalOrdersMetadata', []);
...
window.handleNewORDPATClick = function () {
    ...
    if (refsBody) refsBody.innerHTML = '';
    const orgElementsBody = document.getElementById('opOrgElementsBody');
    if (orgElementsBody) orgElementsBody.innerHTML = '';
    const tareasProgBody = document.getElementById('opTareasProgBody');
    if (tareasProgBody) tareasProgBody.innerHTML = '';
...
function showAppView(viewId) {
    ...
    if (viewId === 'instantOpsView') { renderInstantOpsTable(); populateAnexoBPuestosSelector(); }
...
function handlePersonnelSubmit(e) {
    ...
    const grupoDestino = document.getElementById('pGrupoDestino').value;
    const rotacion = document.getElementById('pRotacion').value;

    if (!validateEcuadorianID(idNum)) {
        showNotification('⚠️ Cédula inválida. Debe tener 10 dígitos y pasar la validación ecuatoriana.');
        return;
    }

    const duplicate = personnel.find(p => p.idNum === idNum && String(p.id) !== String(editingPersonnelId));
    if (duplicate) {
        showNotification('⚠️ Esta cédula ya está registrada en otro personal.');
        return;
    }
...
function applyCodesc2x2Regime(sourceId = 'codescStartDate') {
    ...
    if (diffDays < 0) {
        showNotification('⚠️ La fecha de inicio no puede ser futura.');
        return;
    }
    ...
    personnel.forEach(p => {
        if (p.grupoDestino === 'CODESC') {
            if (p.rotacion === 'ALFA') {
                p.condition = (cycleDay === 0 || cycleDay === 1) ? 'OPERATIVO' : 'FRANCO';
            } else if (p.rotacion === 'BRAVO') {
                p.condition = (cycleDay === 2 || cycleDay === 3) ? 'OPERATIVO' : 'FRANCO';
            }
        }
    });
    ...
    const grupoActivo = (cycleDay < 2) ? 'ALFA' : 'BRAVO';
    showNotification(`✅ Régimen 2x2 aplicado. Grupo Activo hoy: ${grupoActivo} (Día ${cycleDay + 1} del ciclo)`);
}
...
function handleNewPersonnelLoad() {
    if (personnel.length === 0) {
        showNotification("No hay registros actuales para archivar.", "error");
        return;
    }

    if (!confirm("¿Desea archivar la carga actual en el historial y limpiar el registro para una nueva carga? Los datos actuales se conservarán para reportes históricos.")) return;
...
function populateORDPATSelectors() {
    ...
    if (shiftSelector) {
        ...
        const uniqueShifts = [...new Set(all.map(a => a.assignedShift).filter(Boolean))].sort();
        const curShift = shiftSelector.value;
        shiftSelector.innerHTML = '<option value="">-- Seleccionar --</option>';
        ...
        if (postSelector) {
            ...
            postSelector.innerHTML = '<option value="">-- Seleccionar --</option>';
            const filtered = shiftSelector.value ? all.filter(a => a.assignedShift === shiftSelector.value) : all;
            const uniquePosts = [...new Set(filtered.map(a => a.assignedLocation).filter(Boolean))].sort();
...
function editPersonnel(id) {
    ...
    if (document.getElementById('pGrupoDestino')) document.getElementById('pGrupoDestino').value = person.grupoDestino || 'GT ECHO';
    if (document.getElementById('pRotacion')) document.getElementById('pRotacion').value = person.rotacion || 'ALFA';
...
function updatePersonnelStats() {
    ...
    const statGtEcho = document.getElementById('statTotalGtEcho');
    const statCodesc = document.getElementById('statTotalCodesc');
    const statOperativos = document.getElementById('statTotalOperativos');
    const statOtros = document.getElementById('statTotalOtros');

    // Elementos de grupos individuales GT ECHO
    ...
    // 2. FILTRAR PERSONAL POR GRUPO DESTINO (GT ECHO vs CODESC)
    // Usar el campo grupoDestino del personal para clasificación independiente
    const personnelGtEcho = personnel.filter(p => !p.grupoDestino || p.grupoDestino === 'GT ECHO' || p.grupoDestino === 'GT ECHO ');
    const personnelCodesc = personnel.filter(p => p.grupoDestino === 'CODESC' || p.grupoDestino === 'CODESC ');

    // 3. ACTUALIZAR TOTALES GT ECHO Y CODESC (Independientes por grupoDestino)
    if (statGtEcho) statGtEcho.textContent = personnelGtEcho.length;
    if (statCodesc) statCodesc.textContent = personnelCodesc.length;

    // 4. ESTADÍSTICAS DE CONDICIÓN (Operativos vs Descanso)
    ...
    // 5. TOTAL SELECCIONADO (Para la cuadrícula de resumen)
    ...
    if (statGridTotal) statGridTotal.textContent = getSelectedCount();

    // 6. SUBDIVIDIR GRUPOS GT ECHO EN 4 (Alfa, Bravo, Charlie, Delta)
    const totalGtEcho = personnelGtEcho.length;
    const quarterGtEcho = Math.ceil(totalGtEcho / 4);

    const countAlfa = (totalGtEcho > 0) ? Math.min(quarterGtEcho, totalGtEcho) : 0;
    const countBravo = (totalGtEcho > quarterGtEcho) ? Math.min(quarterGtEcho, totalGtEcho - quarterGtEcho) : 0;
    const countCharlie = (totalGtEcho > quarterGtEcho * 2) ? Math.min(quarterGtEcho, totalGtEcho - quarterGtEcho * 2) : 0;
    const countDelta = (totalGtEcho > quarterGtEcho * 3) ? Math.min(quarterGtEcho, totalGtEcho - quarterGtEcho * 3) : 0;

    if (statGtEchoAlfa) statGtEchoAlfa.textContent = countAlfa;
    if (statGtEchoBravo) statGtEchoBravo.textContent = countBravo;
    if (statGtEchoCharlie) statGtEchoCharlie.textContent = countCharlie;
    if (statGtEchoDelta) statGtEchoDelta.textContent = countDelta;

    // 7. SUBDIVIDIR GRUPOS CODESC EN 2 (Foxtrot, Golf)
    const totalCodesc = personnelCodesc.length;
    const halfCodesc = Math.ceil(totalCodesc / 2);

    const countFoxtrot = (totalCodesc > 0) ? Math.min(halfCodesc, totalCodesc) : 0;
    const countGolf = (totalCodesc > halfCodesc) ? Math.min(halfCodesc, totalCodesc - halfCodesc) : 0;

    if (statCodescFoxtrot) statCodescFoxtrot.textContent = countFoxtrot;
    if (statCodescGolf) statCodescGolf.textContent = countGolf;

    // 8. CARGAR DATOS DE TURNOS (Babor/Estribor) SI ESTÁN VACÍOS (para la distribución de guardias)
    if (personnel.length === 0) {
        baborPersonnel = [];
        estriborPersonnel = [];
        guardAssignments = [];
        ...
        // Si hay personal pero no ha sido dividido para turnos, dividir automáticamente
        if (baborPersonnel.length === 0 && estriborPersonnel.length === 0 && personnel.length > 0) {
...
function renderPersonnelTable(searchTerm = '') {
    ...
    filtered.forEach(p => {
        ...
        row.innerHTML = `
            ...
            <td style="${rowColorStyle}">${p.grupoDestino || 'N/A'}</td>
            <td style="${rowColorStyle}">${p.rotacion || 'N/A'}</td>
...
function openOpsModal(eventId = null) {
    ...
    // Población dinámica del select de Distrito basado en Puestos Activos (del listado de distribución)
    if (districtSelect) {
        ...
        const allAssignments = [...(window.guardAssignments || []), ...(window.specialAssignments || [])];
        const uniquePosts = new Set();
        allAssignments.forEach(a => {
            if (a.assignedLocation) uniquePosts.add(a.assignedLocation);
        });

        // Fallback si no hay distribución generada aún
        if (uniquePosts.size === 0) {
            document.querySelectorAll('.post-active:checked, .loc-toggle:checked').forEach(toggle => {
                const val = toggle.getAttribute('data-post') || toggle.value;
                if (val) uniquePosts.add(val);
            });
            const tqT1 = document.getElementById('activateTQT1');
            const tqT2 = document.getElementById('activateTQT2');
            if ((tqT1 && tqT1.checked) || (tqT2 && tqT2.checked)) {
                uniquePosts.add("TOQUE DE QUEDA");
            }
        }
    ...
    if (eventId) {
        ...
        if (event) {
            ...
            if (districtSelect && event.distrito) {
                const existingOption = Array.from(districtSelect.options).some(o => o.value === event.distrito);
                if (!existingOption) {
                    const option = document.createElement('option');
                    option.value = event.distrito;
                    option.textContent = event.distrito;
                    districtSelect.appendChild(option);
                }
        ...
        if (allAssignments.length > 0) {
            // 1. Buscar puesto predominante en las asignaciones
            const locs = allAssignments.map(a => a.assignedLocation).filter(l => l);
            if (locs.length > 0) {
                const counts = locs.reduce((acc, l) => { acc[l] = (acc[l] || 0) + 1; return acc; }, {});
                detectedDistrict = Object.keys(counts).reduce((a, b) => counts[a] > counts[b] ? a : b);
            }

            // 2. Buscar turno predominante en las asignaciones
            const shiftTimes = allAssignments.map(a => a.assignedTime).filter(t => t);
            if (shiftTimes.length > 0) {
                const counts = shiftTimes.reduce((acc, t) => { acc[t] = (acc[t] || 0) + 1; return acc; }, {});
...
function saveData() {
    try {
        ...
        localStorage.setItem('commandPostPersonnel', JSON.stringify(commandPostPersonnel));
        localStorage.setItem('personnelHistory', JSON.stringify(personnelHistory));
...
function populateAnexoBPuestosSelector(skipShiftReset = false) {
    ...
    if (shiftSelector.value) {
        filteredAll = all.filter(a => a.assignedShift === shiftSelector.value);
    }
...
function updateAnexoBPreview() {
    ...
    if (selectedShift) {
        filtered = filtered.filter(a => a.assignedShift === selectedShift);
    }
...
function handleInstantReportSubmit(e) {
    ...
    const newOp = {
        ...
        annexBPersonnel: (() => {
            ...
            if (shift) {
                filtered = filtered.filter(a => a.assignedShift === shift);
            }
...
window.populateOrderReferences = function () {
    ...
    // Suponiendo que patrolOrders es el array global de órdenes
    const orders = window.patrolOrders || [];
    orders.forEach(order => {
        const option = document.createElement('option');
        option.value = order.displayId || order.id || 'S/N';
...
function buildParteAlInstantePDF(doc, op) {
    ...
    doc.autoTable({
        ...
        didDrawPage: function (data) {
            ...
            doc.text('S E C R E T O', pageWidth / 2, 15, { align: 'center' });
            doc.setTextColor(0, 0, 0);
        }
    });
...
function addExtraFixedPost() {
    ...
    div.innerHTML = `
        ...
        <input type="number" class="post-quota" value="1" min="0" style="width:55px; padding: 0.4rem; border: 1px solid var(--border); border-radius: 6px; text-align: center;">
...
function generatePersonnelDistribution() {
    ...
    // Tareas Fijas
    const fixedPosts = document.querySelectorAll('.tactical-sector:first-child .post-item');
    fixedPosts.forEach(item => {
        ...
        if (activeInput && activeInput.checked) {
            ...
            const quota = parseInt(item.querySelector('.post-quota').value) || 0;
            ...
            // Ya no dividimos en T1/T2/T3 automáticamente. 
            // Generamos tantos slots como indique la cantidad (quota) usando el horario manual.
    ...
    // Tareas de Apoyo
    const supportPosts = document.querySelectorAll('.tactical-sector:nth-child(2) .support-post');
    supportPosts.forEach(item => {
        const activeInput = item.querySelector('.post-active');
        if (activeInput && activeInput.checked) {
            const name = activeInput.getAttribute('data-post');
            const time = item.querySelector('.support-time').value || "Horario Manual";
            const qty = parseInt(item.querySelector('.support-qty').value) || 0;
    ...
    let pool = personnel.filter(p => p.condicion === "OPERATIVO" || p.condition === "OPERATIVO");
    pool = pool.sort(() => Math.random() - 0.5); // Mezcla total inicial

    const gradePools = {};
    fullHierarchy.forEach(g => {
        gradePools[g] = pool.filter(p => p.grade === g);
    });

    guardAssignments = [];
    let unassignedPersonnel = [...pool];

    // --- FASE 1: GARANTÍA DE OFICIALES POR TURNO/PUESTO ---
    // Agrupar slots por combinación única de (Lugar + Turno + Horario)
    const uniqueShiftGroups = {};
    allSlots.forEach(slot => {
        const key = `${slot.locName}|${slot.shift}|${slot.time}`;
        if (!uniqueShiftGroups[key]) uniqueShiftGroups[key] = [];
        uniqueShiftGroups[key].push(slot);
    });

    let insufficientOfficials = false;

    for (const key in uniqueShiftGroups) {
        const slotsInGroup = uniqueShiftGroups[key];
        // Garantizar al menos un oficial por puesto activo
        let officer = null;
        for (const grade of OFFICERS_GRADES) {
            if (gradePools[grade] && gradePools[grade].length > 0) {
                officer = gradePools[grade].shift();
                break;
            }
        }

        if (officer) {
            const slotToFill = slotsInGroup[0];
            slotToFill.assigned = true;
            guardAssignments.push({
                ...officer,
                assignedLocation: slotToFill.locName,
                assignedShift: slotToFill.shift,
                assignedTime: slotToFill.time
            });
            unassignedPersonnel = unassignedPersonnel.filter(pers => (pers.id || pers.dni) !== (officer.id || officer.dni));
        } else {
            insufficientOfficials = true;
        }
    }

    if (insufficientOfficials) {
        setDistributionInterfaceMessage('No hay suficientes oficiales para cubrir cada sitio activo. La distribución se generará con el personal disponible, pero revisa la lista de oficiales.');
    }

    // --- FASE 2: DISTRIBUCIÓN EQUITATIVA DEL RESTO ---
    let remainingSlots = allSlots.filter(s => !s.assigned);
    let gradeKeys = fullHierarchy.filter(g => gradePools[g].length > 0);
    let gradeIdx = 0;
    ...
    remainingSlots.forEach(slot => {
        ...
        let attempts = 0;
        while (attempts < gradeKeys.length) {
            const currentGrade = gradeKeys[gradeIdx % gradeKeys.length];
            const gPool = gradePools[currentGrade];
            ...
            if (gPool.length > 0) {
                ...
                slot.assigned = true;
                unassignedPersonnel = unassignedPersonnel.filter(pers => (pers.id || pers.dni) !== (p.id || p.dni));

                if (gPool.length === 0) {
                    gradeKeys = gradeKeys.filter(g => g !== currentGrade);
                } else {
                    gradeIdx++;
                }
                break;
    ...
    // FASE 3: Personal excedente (Mantener como DISPONIBLE)
    ...
    sortPoolByRank(remainingPersonnel);
    remainingPersonnel.forEach((p) => {
        if (!isAlreadyAssigned(p)) {
            markAsAssigned(p);
            guardAssignments.push({
                ...p,
                assignedLocation: "DISPONIBLE",
                assignedShift: "DISPONIBILIDAD",
                assignedTime: "N/A"
            });
        }
    });
    ...
    showNotification("Distribución generada con garantía de liderazgo.");
}
...
function renderDistributionTable() {
    ...
    sortedLocations.forEach(locName => {
        ...
        let catColor = "#3b82f6"; // Azul por defecto (Tareas Fijas)
        let catBg = "rgba(59, 130, 246, 0.1)";

        const nameUpper = locName.toUpperCase();
        if (nameUpper.includes("FRANCO")) {
            catColor = "#8b5cf6"; // Morado para Franquicia
        ...
        headerRow.style.background = catBg;
        headerRow.innerHTML = `
            <td colspan="7" style="color: ${catColor}; font-weight: 800; padding: 12px 20px; border-left: 6px solid ${catColor}; font-family: 'Outfit', sans-serif;">
                ${nameUpper}
            </td>
            <td style="text-align: right; color: ${catColor}; font-weight: 700; font-size: 0.85em; padding-right: 20px;">
        ...
        Object.keys(timeGroups).forEach(time => {
            ...
            // Ordenar personal por jerarquía (de mayor a menor)
            const OFFICERS_GRADES = ["CPNV", "CPFG", "CPCB", "TNNV", "TNFG", "ALFG"];
            const OTHER_GRADES = ["SUBM", "SUBP", "SUBS", "SGOP", "SGOS", "CBOP", "CBOS", "MARO"];
            const rankHierarchy = [...OFFICERS_GRADES, ...OTHER_GRADES];

            timeMembers.sort((a, b) => {
                let rankA = rankHierarchy.indexOf(a.grade);
                let rankB = rankHierarchy.indexOf(b.grade);
            ...
            timeMembers.forEach(item => {
                ...
                const rotacionText = item.rotacion || 'N/A';
                let rotColor = '#64748b'; // N/A
                if (rotacionText === 'ALFA') rotColor = '#3b82f6';
                else if (rotacionText === 'BRAVO') rotColor = '#10b981';
                else if (rotacionText === 'CHARLIE') rotColor = '#f59e0b';
                else if (rotacionText === 'DELTA') rotColor = '#8b5cf6';

                const rotBadgeHtml = rotacionText !== 'N/A'
                    ? `<span style="padding:2px 7px; border-radius:8px; font-size:0.75rem; font-weight:700; color:white; background-color:${rotColor};">${rotacionText}</span>`
                ...
                row.innerHTML = `
                    ...
                    <td style="text-align: center;">${rotBadgeHtml}</td>
                    <td style="text-align: center;">
...
function renderHistoricalPatrolTable() {
    ...
    patrolOrders.forEach(order => {
        const row = document.createElement('tr');
        row.innerHTML = `
            <td style="font-weight: 700; color: var(--text-main);">${order.num}</td>
            <td>${order.unit}</td>
...
function renderORDPATTable() {
    ...
    sorted.forEach((op, index) => {
        ...
        // Manejo de estado por defecto 'abierta'
        const isAbierta = !op.status || op.status === 'abierta';
        const statusBadge = isAbierta
            ? `<span style="background: #fef08a; color: #854d0e; padding: 3px 6px; border-radius: 4px; font-weight: bold; font-size: 0.7rem; display: inline-block; min-width: 60px; text-align: center;">ABIERTA</span>`
            : `<span style="background: #e2e8f0; color: #475569; padding: 3px 6px; border-radius: 4px; font-weight: bold; font-size: 0.7rem; display: inline-block; min-width: 60px; text-align: center;">CERRADA</span>`;
        ...
        row.innerHTML = `
            ...
            <td>${op.dtg || '---'}</td>
            <td style="font-size: 0.8rem; max-width: 250px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${op.mision || '---'}</td>
            <td>${statusBadge}</td>
            <td class="table-actions" style="display: flex; gap: 5px; justify-content: center; min-width: 140px;">
                <button class="btn-action edit" data-id="${op.id}" title="Generar PDF Oficial">📄</button>
                ${isAbierta
                ? `<button class="btn-action" style="background: var(--accent-primary); border-color: var(--accent-primary); color: white;" onclick="openCloseOrderModal('${op.id}')" title="Subir Cumplimiento y Cerrar">🔒 Cerrar</button>`
                : `<button class="btn-action" style="background: #0f172a; border-color: #0f172a; color: white;" onclick="viewCompliancePDF('${op.id}')" title="Ver Cumplimiento Subido">📑 Cumplimiento</button>`}
...
function addPersonnelAnnex(doc, pageWidth, filterShift = null, filterPost = null, fontSize = 8, itemsOverride = null, docTitle = "ORDEN DE PATRULLA") {
...
async function generateORDPATPDF(id, options = {}) {
    ...
    const renderJustifiedBlock = (text, x, maxWidth, indent = 0) => {
    ...
    renderJustifiedBlock(op.mision || '---', margin, pageWidth - (2 * margin), 5);
    currentY += 5;

    drawSectionHeader("3. EJECUCIÓN");
    doc.setFont('helvetica', 'bold');
    doc.text("A. CONCEPTO DE LA OPERACIÓN", margin + 5, currentY);
    currentY += 7;
    doc.setFont('helvetica', 'normal');
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(op.firmanteNombre.toUpperCase(), margin + porW, currentY);
    currentY += 5;

    // Linea 2: Grado
    doc.text(op.firmanteGrado.toUpperCase(), margin, currentY);
    currentY += 5;

    // Linea 3: Cargo (Negrita)
    doc.setFont('helvetica', 'bold');
    doc.text(op.firmanteCargo.toUpperCase(), margin, currentY);
    currentY += 8;

    // Linea 4: Elaborador
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
    const authorLine = `${op.autorNombre.toUpperCase()} — ${op.autorCargo.toUpperCase()}`;
    doc.text(authorLine, margin, currentY);
    currentY += 10;
    ...
    currentY += 5;
    doc.text('B: REGISTRO FOTOGRÁFICO', margin + anexW, currentY);

    addPersonnelAnnex(doc, pageWidth, null, null, 8, op.personnelSnapshot, "ORDEN DE PATRULLA");
    ...
    for (let i = 1; i <= totalPages; i++) {
        ...
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(190, 30, 45); // Rojo Institucional
        doc.text('SECRETO', margin, headerY);

        // Bloque Unidad Top-Right
        doc.setTextColor(0, 0, 0);
        doc.setFontSize(9);
        const rightX = pageWidth - margin;
        doc.text('ARMADA DEL ECUADOR', rightX, headerY, { align: 'right' });
        headerY += 4;
        doc.text('GRUPO DE TAREA 100.51', rightX, headerY, { align: 'right' });
        headerY += 4;
        doc.text('“SEGURIDAD MARÍTIMA”', rightX, headerY, { align: 'right' });
        headerY += 4;
        doc.text(op.lugar || 'GUAYAQUIL', rightX, headerY, { align: 'right' });
        headerY += 4;
        doc.text(new Date().toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' }).toUpperCase(), rightX, headerY, { align: 'right' });

        if (escudoBase64) {
            const shieldSize = 20;
            doc.addImage(escudoBase64, 'PNG', (pageWidth - shieldSize) / 2, 12, shieldSize, shieldSize);
        }

        // Título y ID de Orden (A partir de la página 2 si es necesario, o en todas)
        headerY = 45;
        doc.setFontSize(12);
        doc.setFont('helvetica', 'bold');
        ...
        headerY += 7;
        const ordIdLoop = `${op.prefix || 'ARE-ORDPAT-UT100.51.4-'}${op.dtg || 'XXXXXXR-XXX-2026'}-${op.serial || 'XXX'}-S`;
        doc.setFontSize(9);
        doc.text(ordIdLoop, pageWidth / 2, headerY, { align: 'center' });

        // Clasificación al Pie
        doc.setFontSize(14);
        doc.setFont(undefined, 'bold');
...
// --- Initialization for Intelligence Filters ---
document.addEventListener('DOMContentLoaded', () => {
